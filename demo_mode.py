...
class DemoModeManager:
    ...
    def banner_html(self) -> str:
        if not self.is_demo:
            return ""
        return (
            "<div class=\"sticky top-0 z-[60] border-b border-blue-100 bg-blue-50/95 "
            "backdrop-blur supports-[backdrop-filter]:bg-blue-50/80\">"
            "<div class=\"container flex flex-col gap-4 px-4 py-3 text-blue-900 "
            ...
            "</span>"
            "<div class=\"space-y-1\">"
            "<p class=\"text-sm font-semibold text-foreground\">Demo Mode</p>"
            "<p class=\"text-sm text-muted-foreground\">You are viewing sample data. "
            "Connect your accounts to use real data.</p>"
            "</div>"
            "</div>"
            "<a href=\"/admin/mode\" class=\"inline-flex items-center justify-center rounded-md "
            "bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow-sm transition "
