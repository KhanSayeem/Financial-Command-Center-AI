...
class MCPTester:
    ...
    def _prepare_calls(self, tools: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        next_id = 3
        for tool in tools:
            if tool.get("name") == "capture_payment_intent":
    ...
    def _generate_arguments(self, schema: Dict[str, Any], tool_name: str, description: str) -> Dict[str, Any]:
        ...
        for name, prop_schema in properties.items():
            if name in stripe_overrides:
                arguments[name] = stripe_overrides[name]
                continue
...
def main() -> int:
    ...
    if stripe_key:
        config["mcpServers"]["stripe-payments"]["env"]["STRIPE_API_KEY"] = stripe_key
